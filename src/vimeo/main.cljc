(ns vimeo.main
  "Kotodama WASM entrypoint for the Vimeo clean-room actor (L5) — Clojure port.
  ...
  facts (`vimeo.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Video"    :plural "videos"    :id-prefix "vimeo_vid"
    :fields [:name :description :duration :width :height :language :license :status :link :releaseTime :resourceKey :uri]
    :required [:name :description]
    :coerce {:duration :float :width :int :height :int} :refs {}}
   {:entity "Channel"  :plural "channels"  :id-prefix "vimeo_cha"
    :fields [:name :description :link :resourceKey :uri] :required [:name :description]
    :coerce {} :refs {}}
   {:entity "Album"    :plural "albums"    :id-prefix "vimeo_alb"
    :fields [:name :description :duration :layout :sort :theme :brandColor :allowDownloads :allowShare :allowContinuousPlay :reviewMode :hideNav :link :url :resourceKey :uri]
    :required [:name :description]
    :coerce {:duration :float :allowDownloads :bool :allowShare :bool :allowContinuousPlay :bool :reviewMode :bool :hideNav :bool} :refs {}}
   {:entity "User"     :plural "users"     :id-prefix "vimeo_use"
    :fields [:name :bio :email :location :account :link :resourceKey :uri] :required [:name :bio]
    :coerce {} :refs {}}
   {:entity "Group"    :plural "groups"    :id-prefix "vimeo_gro"
    :fields [:name :description :link :resourceKey :uri] :required [:name :description]
    :coerce {} :refs {}}
   {:entity "Category" :plural "categories" :id-prefix "vimeo_cat"
    :fields [:name :topLevel :lastVideoFeaturedTime :link :resourceKey :uri] :required [:name :topLevel]
    :coerce {:topLevel :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"vimeo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vimeo-compat" :tier tier :entities entities} 200])
